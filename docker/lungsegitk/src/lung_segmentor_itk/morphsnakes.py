...
_curvop = _fcycle([lambda u: sup_inf(inf_sup(u)), lambda u: inf_sup(sup_inf(u))])  # SIoIS  # ISoSI
...
def morphological_chan_vese(
    image, iterations, init_level_set="checkerboard", smoothing=1, lambda1=1, lambda2=1, iter_callback=lambda x: None
):
...
def morphological_geodesic_active_contour(
    gimage, iterations, init_level_set="circle", smoothing=1, threshold="auto", balloon=0, iter_callback=lambda x: None
):
